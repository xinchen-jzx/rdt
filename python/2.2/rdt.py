    ...
    def rdt_send(self, data):
        ...
        while True:
            ...
            if recv_p['seq'] == self.last_seq:
                print(f"错序包: recv seq: {recv_p['seq']}, resend: {self.seq}")
                udt_send(send_pkt, self.addr, self.paddr)
                continue
            # 好包 and 正序包 -> 进入下一个等待状态
            elif (not isco) and (recv_p['seq'] == self.seq):
                print(f"好包: recv seq: {recv_p['seq']}, sended: {self.seq}")
                break
            else:
...
class Receiver:
    ...
    def rdt_recv(self):
        while True:
            recv_pkt = udt_recv(self.addr)
            recv_pkt = random_bits_err(recv_pkt)
            isco = is_corrupt(recv_pkt)
            # 坏包 -> 重传, 不转移
            if isco:
                print(
