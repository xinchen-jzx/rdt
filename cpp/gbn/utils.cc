...
{
    if ((size > DATA_MAX_SIZE) || (buffer_size < sizeof(Packet)))
    {
...
uint32_t cal_checksum(const Packet *p)
{
    ...
    for (int i = CHECKSUM_START; i < sizeof(Packet); i++)
    {
        checksum += pp[i];
    }
    return checksum;
...
void random_bit_err(Packet *&p, const double rate)
{
    random_device r;
